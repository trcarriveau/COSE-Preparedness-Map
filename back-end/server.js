...
const db = mongoose.connect("mongodb://localhost:27017/COSE-Preparedness-Map", {
...
app.get("/user_access", async (req, res) => {
  console.log("Hit user_access");
  try {
    let userList = await Users.find({}, "username role").exec();
    res.json(userList);
  } catch (error) {
    console.log("In user_access --> received error");
  }
});
